...
export const create = async (
  data: (typeof CreateObligationSchema)["_input"]
) => {
  try {
    ...
    if (!data.tender || !data.contract) {
      return {
        success: false,
        message: "No target provided for the obligation",
      }
    }
    ...
    revalidatePath(
      obligation.tender
        ? `/tenders/${obligation.tender}/obligations`
        : `/contracts/${obligation.contract}/obligations`
    )
    return {
      success: true,
      data: obligation,
